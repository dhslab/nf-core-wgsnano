...
workflow WGSNANO {
    ...
    INPUT_CHECK
        .out
        .reads
        .map { meta, fast5_path -> [meta, file("${fast5_path}/*.fast5")] }
        .flatMap { meta, files ->
            def chunks = files.toList().collate(2)  // chunk files into groups of 2
            def chunkList = []
    ...
    // .flatMap { meta, files ->
    //     def chunks = files.toList().collate(2)  // chunk files into groups of 2
    //     return chunks.collect { [meta, it] }  // associate each chunk with the same meta
    // }
    // .dump(pretty: true)
    // .set { ch_fast5_chunks }

    // Convert fast5 to pod5 in chunks
    FAST5_TO_POD5 {
        ch_fast5_chunks
    }
    ...
    if (params.reads_format == 'fast5') {
        ...
        //
        // INPUT_CHECK.out.reads.dump(pretty: true)
